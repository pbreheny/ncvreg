...
where $L(\bb|\X,\y)$ is the loss (typically, the negative log-likelihood) and $P_\lam(\bb)$ is the penalty.  This article describes the different penalties available in `ncvreg`; see [models](models.html) for more information on the different loss functions available.  Throughout, linear regression and the `Prostate` data set is used
