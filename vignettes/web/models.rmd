...
$$
this loss is the negative log-likelihood if the outcome $\y$ follows a binomial distribution with probabilities given by:
$$
...
$$
L(\bb|\X,\y) = \sum_i \{y_i\log\mh_i - \mh_i\};
$$
this loss is the negative log-likelihood if the outcome $\y$ follows a Poisson distribution with rate parameter given by:
$$
\mh_i = \exp(\eta_i).
$$
...
$$
L(\bb|\X,\y) = \sum_{j=1}^{m} d_j \eta_j - \sum_{j=1}^{m} d_j \log\left\{\sum_{i \in R_j} \exp(\eta_i)\right\},
$$
