...
- New: intervals() to get intervals around estimates (also supports
  debiased intervals)
...
- New: summary.ncvreg() now offers sort option; fixes
  [\#13](https://github.com/pbreheny/ncvreg/issues/13)
- Change: fir() deprecated
- Change: local_mfdr() allows user to specify sigma; also uses CV if
  called with cv object
- Fixed: Manual color palettes now recycled correctly; fixes
  [\#40](https://github.com/pbreheny/ncvreg/issues/40); thank you to
  Logan Harris for pointing this out
...
- Fixed: Memory leak in cox-dh; resolves
  [\#20](https://github.com/pbreheny/ncvreg/issues/20)
...
CRAN release: 2018-09-13

- Change: returnX now turned on by default if X \< 100 Mb (used to be 10
  Mb)
...
- New: summary.ncvreg and summary.ncvsurv now report tables of inference
  for each feature based on local mFDRs
...
- Change: All data sets now follow DataX, Datay convention
...
- New: Added function AUC() to calculate cross-validated AUC values for
  ncvsurv models.
...
- Internal: Substantial efficiency improvements throughout for Cox
  models. Coordinate descent redesigned to work in O(n) instead of
  O(n^2) operations, and R code redesigned at various points to avoid
  the creation of any n x n matrices when fitting and cross-validating
...
CRAN release: 2014-02-25

- New: Added support for Poisson regression
- Fixed: Bug in ncvreg_fit that could arise when fitting a model without
  an intercept
...
CRAN release: 2014-02-06

- New: Added fir, perm.ncvreg, and plot.fir functions for the purposes
  of estimating and displaying false inclusion rates; these are likely
  to evolve over the next few months
- Fixed: Bug in cv.ncvreg for user-specified lambda sequence
- Internal: Revised algorithms to incorporate targeted cycling based on
  strong rules
...
- Fixed: Bug in axis annotations with plot.cv.ncvreg when model is
  saturated
- Fixed: Deviance calculation; would return NaN if fitted probabilities
  of 0 or 1 occurred for binomial outcomes
- Fixed: NAMESPACE for coef.cv.ncvreg and predict.cv.ncvreg
- Internal: .Call now used instead of .C

## ncvreg 2.5-0

CRAN release: 2013-03-16
...
- New: Summary method for cv.ncvreg which displays the above information
  at lambda.min, the value of lambda minimizing the cross-validation
  error
...
- Internal: cdfit\_ now returns loss (RSS for gaussian, deviance for
  binomial)

## ncvreg 2.3-2

CRAN release: 2011-05-16

- Documentation: Fixed formatting error in citation.

## ncvreg 2.3-1

CRAN release: 2011-05-11
...
CRAN release: 2011-05-06

- Documentation: Updated documentation for cv.ncvreg.Rd, which no longer
  agreed with the function usage (this was an oversight in the release
  of version 2.2)

## ncvreg 2.2

CRAN release: 2011-04-25
...
- Changed: Divorced cross-validation from fitting in cv.ncvreg. From a
  ...
  model with CV-chosen regularization parameter now requires two calls
  (to ncvreg and cv.ncvreg). The functions, however, are logically
  separate and involve entirely separate methods.
