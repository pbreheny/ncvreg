...
run_tests <- function(boot_res) {
  tinytest::expect_inherits(boot_res, "list")
...
run_tests(boot_res)
tinytest::expect_equal(dim(boot_res$boot_draws), c(1000, 10))

## Pass in CV object
cv_fit <- cv.ncvreg(X, y, penalty = "lasso")
boot_res <- boot_ncvreg(fit = cv_fit, returnCV = TRUE)
run_tests(boot_res)
tinytest::expect_equivalent(cv_fit, boot_res$cv.ncvreg)
...
tinytest::expect_error(boot_ncvreg(X))
...
tinytest::expect_identical(seed_before, seed_after)

## Pass in CV object and X and y, expect error as passed X is made different
cv_fit <- cv.ncvreg(X, y, penalty = "lasso")
tinytest::expect_error(boot_ncvreg(matrix(rnorm(500), 50, 10), y, fit = cv_fit))
...
cv_fit <- cv.ncvreg(X, y, penalty = "lasso")
tinytest::expect_error(boot_ncvreg(y = X[,1] + rnorm(50), fit = cv_fit))

## Pass in CV object with no X (expect error)
cv_fit <- cv.ncvreg(X, y, penalty = "lasso", returnX = FALSE)
tinytest::expect_error(boot_res <- boot_ncvreg(fit = cv_fit))
...
## Coercion 
tinytest::expect_error(boot_ncvreg(list(X), y))
...
## Misc Checks
tinytest::expect_warning(boot_ncvreg(X, fit = cv_fit, nlambda = 10), strict = TRUE)
tinytest::expect_error(boot_ncvreg(X, y, penalty.factor = c(0, rep(1, 9))))
tinytest::expect_error(boot_ncvreg(X, y, family = "poisson"))
tinytest::expect_error({
  y_bin <- exp(y) / (1+exp(y)) > 0.5
  cv_fit <- cv.ncvreg(X, y_bin, family = "binomial")
  boot_ncvreg(fit = cv_fit)
})
tinytest::expect_message(boot_ncvreg(X, y, verbose = TRUE), strict = TRUE)
tinytest::expect_message(boot_ncvreg(X, y, verbose = TRUE, sigma2 = 1, lambda = 0.1), strict = TRUE)
tinytest::expect_message(boot_ncvreg(X, y, returnX = TRUE, convex = TRUE), strict = TRUE)
...
tinytest::expect_true(runtime_par[[2]] < runtime_orig[[2]])

cv_fit <- cv.ncvreg(X, y, penalty = "lasso")
tinytest::expect_error(boot_ncvreg(fit = cv_fit, cluster = NA)) ## Passing something other than a cluster
