...
#'
#' Constructs projection based test statistics that can be used to control FDR along with intervals
#' for a penalized regression model.
#'
#' The function constructs test statistics and intervals based off an approximate projection onto
#' the column space of the active features. The test statistic can be used to control FDR and the
...
#' @param lambda The penalty at which the tests and intervals are to be constructed. If left
#'   unspecified, will be selected using cross validation.
#' @param sigma Standard deviation estimate used to compute test statistic and intervals. If left
#'   unspecified (default) it will be estimated using the recommendation from Reid et al. (2016)
#' @param level the confidence level required.
#' @param posterior whether the intervals returned should be posterior intervals (default) or
#'   debiased intervals (if `FALSE`). Posterior intervals are constructed from distributions where
#'   the coefficient estimates are the is the posterior mode. Debiased intervals are constructed
#'   around the estimates.
#' @param relaxed whether the relaxed lasso based statistic / intervals should be used. Default is
#'   `FALSE` in which case PIPE based intervals are constructed (recommended). This affects the
#'   estimate.
#' @param adjust_projection whether a Local Quadratic Approximation should be used in the projection
#'   for determining variance of the PIPE based test statistic. Default is `FALSE` as more research
#'   has been done without this adjustment, however without this adjustment, statistics and
#'   intervals may be over conservative in the presence of correlation. This affects the SE.
#' @param X The original design matrix supplied to `fit`. Required if `fit` does not contain `X`.
#'
...
#' - Harris L and Breheny P. (2025) Alternative Likelihood Approximations for High-Dimensional
#'   Intervals for Lasso. *arXiv preprint*, arXiv:2509.14971.
...
intervals <- function(
  ...
  if (inherits(fit, "cv.ncvreg")) {
    ...
    if (lambda == max(cv_fit$lambda)) {
      lambda <- lambda * 0.999
    }
    if (lambda == min(cv_fit$lambda)) {
      lambda <- lambda * 1.001
    }
  }
  ...
  if (fit$family == "gaussian") {
    ...
    if (length(S_hat) > 0 & length(N_hat) > 0) {
      ...
      if (!adjust_projection) {
        ...
        Qs <- diag(nrow(XX)) - Xs %*% solve(((1 / n) * crossprod(Xs) + adjs)) %*% ((1 / n) * t(Xs))
      }
    } else {
      Qs <- diag(nrow(XX))
    }
    ...
    for (i in S_hat) {
      ...
      if (relaxed) {
        ...
        beta_PIPE[i] <- (t(XX[, i, drop = FALSE]) %*% (yy - Xsi %*% beta[c(TRUE, S_hat_i)])) /
    ...
    for (i in N_hat) {
      if (relaxed) {
        ...
        beta_PIPE[i] <- (t(XX[, i, drop = FALSE]) %*% (yy - Xs %*% beta[c(TRUE, S)])) /
          (t(XX[, i, drop = FALSE]) %*% XX[, i, drop = FALSE])
      }
...
penalty_derivative <- function(
  ...
  d_sparse_raw <- ifelse(
    penalty == "lasso",
    lambda,
    ifelse(penalty == "MCP", mu_mcp, ifelse(penalty == "SCAD", mu_scad, NA_real_))
  )
...
log_norm_masses_mcp <- function(mu, s2, lambda, gamma) {
  ...
  log_mass_neg_in <- log(pnorm(0, mean = m_neg, sd = sd_in) - pnorm(-thr, mean = m_neg, sd = sd_in))

  # Positive inside: N(m_pos, v_in) over [0, thr]
  log_mass_pos_in <- log(pnorm(thr, mean = m_pos, sd = sd_in) - pnorm(0, mean = m_pos, sd = sd_in))

  # Positive outside: N(mu, s2) over (thr, Inf)
  log_mass_pos_out <- pnorm(thr, mean = mu, sd = sd_out, lower.tail = FALSE, log.p = TRUE)
...
log_norm_masses_scad <- function(mu, s2, lambda, gamma) {
  ...
  log_neg_mid <- log(
    pnorm(-thr1, mean = m_m_neg, sd = sd_m) - pnorm(-thr2, mean = m_m_neg, sd = sd_m)
  )
  log_pos_mid <- log(
    pnorm(thr2, mean = m_m_pos, sd = sd_m) - pnorm(thr1, mean = m_m_pos, sd = sd_m)
  )

  log_neg_in <- log(pnorm(0, mean = m_i_neg, sd = sd_i) - pnorm(-thr1, mean = m_i_neg, sd = sd_i))
  log_pos_in <- log(pnorm(thr1, mean = m_i_pos, sd = sd_i) - pnorm(0, mean = m_i_pos, sd = sd_i))
...
log_constants_scad <- function(mu, s2, lambda, gamma) {
  ...
  m_m_neg <- (mu + cshift * lambda) / A
  m_m_pos <- (mu - cshift * lambda) / A
  logC_nm <- (m_m_neg^2 / (2 * v_m)) -
    (mu^2 / (2 * s2)) +
    (lambda^2 / (2 * (gamma - 1) * s2)) -
    logZ_m
  logC_pm <- (m_m_pos^2 / (2 * v_m)) -
    (mu^2 / (2 * s2)) +
    (lambda^2 / (2 * (gamma - 1) * s2)) -
    logZ_m
...
posterior_quantile <- function(
  ...
  worker <- function(mu_i, s2_i, lam_i, p_i) {
    if (penalty == "lasso") {
      U <- log_norm_masses_lasso(mu_i, s2_i, lam_i)
      C <- log_constants_lasso(mu_i, s2_i, lam_i)
      ...
      C <- log_constants_mcp(mu_i, s2_i, lam_i, gamma)
      P <- posterior_component_proportions(U, C)
      cumP <- cumsum(P)
      weights <- exp(U - log(P))
      ...
      if (k == 1) {
        q <- qnorm(total_prop_k, mean = mu_i, sd = sd_out)
      } else if (k == 2) {
        p_loc <- total_prop_k + pnorm(-thr, m_neg, sd_in)
        q <- qnorm(p_loc, mean = m_neg, sd = sd_in)
      } else if (k == 3) {
        p_loc <- total_prop_k + pnorm(0, m_pos, sd_in)
        q <- qnorm(p_loc, mean = m_pos, sd = sd_in)
      } else {
        p_loc <- total_prop_k + pnorm(thr, mu_i, sd_out)
        q <- qnorm(p_loc, mean = mu_i, sd = sd_out)
      }
    } else if (penalty == "SCAD") {
      U <- log_norm_masses_scad(mu_i, s2_i, lam_i, gamma)
      C <- log_constants_scad(mu_i, s2_i, lam_i, gamma)
      P <- posterior_component_proportions(U, C)
      cumP <- cumsum(P)
      weights <- exp(U - log(P))
      ...
      if (k == 1) {
        q <- qnorm(total_prop_k, mean = mu_i, sd = sd_o)
      } else if (k == 2) {
        p_loc <- total_prop_k + pnorm(-thr2, mean = m_m_neg, sd = sd_m)
        q <- qnorm(p_loc, mean = m_m_neg, sd = sd_m)
      } else if (k == 3) {
        p_loc <- total_prop_k + pnorm(-thr1, mean = m_i_neg, sd = sd_i)
        q <- qnorm(p_loc, mean = m_i_neg, sd = sd_i)
      } else if (k == 4) {
        p_loc <- total_prop_k + pnorm(0, mean = m_i_pos, sd = sd_i)
        q <- qnorm(p_loc, mean = m_i_pos, sd = sd_i)
      } else if (k == 5) {
        p_loc <- total_prop_k + pnorm(thr1, mean = m_m_pos, sd = sd_m)
        q <- qnorm(p_loc, mean = m_m_pos, sd = sd_m)
      } else {
        p_loc <- total_prop_k + pnorm(thr2, mean = mu_i, sd = sd_o)
...
posterior_ci <- function(z, se, lambda, gamma, penalty, alpha, enet_alpha = 1, weights = NULL) {
  if (enet_alpha < 1) {
