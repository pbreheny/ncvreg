...
#'
#' Similar to other predict methods, this function returns predictions from a fitted `ncvsurv`
#' object.
#'
#' Estimation of baseline survival function conditional on the estimated values of `beta` is carried
#' out according to the method described in Chapter 4.3 of Kalbfleish and Prentice. In particular,
#' it agrees exactly the results returned by `survfit.coxph(..., type='kalbfleisch-prentice')` in
#' the `survival` package.
#'
#' @param object Fitted `"ncvsurv"` model object.
#' @param X Matrix of values at which predictions are to be made. Not used for `type="coefficients"`
#'   or for some of the `type` settings in `predict`.
...
#'
#' @returns
#' The return value depends on `type`. Unless otherwise noted, predictions are returned as a matrix
#' with one row per observation in `X` (or per observation in the original dataset if `X` is not
#' provided) and one column per value of `lambda`. If `lambda` has length 1, the result is
#' simplified to a vector with one element per observation.
#'
#' \describe{
#'   \item{`type = "link"`}{A numeric matrix of linear predictors.}
#'   \item{`type = "response"`}{A numeric matrix of estimated risks.}
#'   \item{`type = "survival"`}{
#'     A list of estimated survival functions (if `type = "survival"`, `lambda` must be a scalar).
#'   }
#'   \item{`type = "median"`}{A numeric matrix of estimated median survival times.}
#'   \item{`type = "coefficients"`}{
#'     A numeric matrix of estimated coefficients. One row per column of `X` and one column per
#'     value of `lambda`.
#'   }
#'   \item{`type = "vars"`}{
#'     A list with an element for each `lambda`. Each element is a named vector of integers
#'     corresponding to the indices with nonzero coefficients.
#'   }
#'   \item{`type = "nvars"`}{
#'     A numeric vector with an element for each value of `lambda` indicating the number of nonzero
#'     coefficients.
#'   }
#' }
#'
...
predict.ncvsurv <- function(
  ...
  if (!missing(lambda)) {
    W <- (1 - x) *
      exp(object$linear.predictors)[, l, drop = FALSE] +
  ...
  if (type %in% c("survival", "hazard")) {
    val <- vector("list", length(eta))
  }
  if (type == "median") {
    val <- matrix(NA, nrow(eta), ncol(eta))
  }
  for (j in 1:ncol(eta)) {
    ...
    for (i in 1:nrow(eta)) {
      ...
      H <- H0 * exp(eta[i, j])
      if (type == "survival") {
        val[[i]] <- approxfun(x, S, method = "constant", ties = "ordered")
      } else if (type == "hazard") {
        val[[i]] <- approxfun(x, H, method = "constant", ties = "ordered")
      } else if (type == "median") {
        if (any(S < 0.5)) {
