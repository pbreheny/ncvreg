...
#'
#' Fit coefficients paths for MCP- or SCAD-penalized regression models over a grid of values for the
#' regularization parameter lambda. Fits linear and logistic regression models, with option for an
#' additional L2 penalty.
#'
#' The sequence of models indexed by the regularization parameter `lambda` is fit using a coordinate
#' descent algorithm. For logistic regression models, some care is taken to avoid model saturation;
#' the algorithm may exit early in this setting. The objective function is defined to be
#'
#' \deqn{Q(\beta|X, y) = \frac{1}{n} L(\beta|X, y) + P_\lambda(\beta),}
#'
#' where the loss function L is the deviance (-2 times the log likelihood) for the specified outcome
#' distribution (gaussian/binomial/poisson). See
#' [here](https://pbreheny.github.io/ncvreg/articles/models.html) for more details.
#'
#' This algorithm is stable, very efficient, and generally converges quite rapidly to the solution.
#' For GLMs,
#' [adaptive rescaling](https://pbreheny.github.io/ncvreg/articles/adaptive-rescaling.html) is used.
#'
#' @param X The design matrix, without an intercept. `ncvreg` standardizes the data and includes an
...
#'   predictors for each coefficient is less than `eps`. Default is `1e-4`.
...
#'   can be 0, in which case the coefficient is always in the model without shrinkage.
...
#'   }
#'   \item{loss}{
#'     A vector containing the deviance (i.e., the loss) at each value of `lambda`. Note that for
#'     `gaussian` models, the loss is simply the residual sum of squares.
#'   }
#'   \item{n}{Sample size.}
#' }
#'
...
ncvreg <- function(
  ...
  if (!is.null(ncol(y)) && ncol(y) > 1) {
    stop("y should be a vector of responses, not a matrix", call. = FALSE)
  }
  if (!is.double(y)) {
    op <- options(warn = 2)
    on.exit(options(op))
  ...
  if (length(penalty.factor) != ncol(X)) {
    stop("Dimensions of penalty.factor and X do not match", call. = FALSE)
  }
  ...
  ## Deprication support
  dots <- list(...)
  if ("n.lambda" %in% names(dots)) {
    nlambda <- dots$n.lambda
  }
  ...
  n <- length(yy)
  if (missing(lambda)) {
    lambda <- setup_lambda(XX, yy, family, alpha, lambda.min, nlambda, penalty.factor)
    user.lambda <- FALSE
  } else {
  ...
  convex.min <- if (convex) {
    convex_min(b, XX, penalty, gamma, lambda * (1 - alpha), family, penalty.factor, a = a)
  } else {
    NULL
  }
