...
#'
#' Performs k-fold cross validation for MCP- or SCAD-penalized regression models over a grid of
#' values for the regularization parameter lambda.
#'
#' The function calls `ncvreg` /`ncvsurv` `nfolds` times, each time leaving out 1/`nfolds` of the
#' data. The cross-validation error is based on the deviance;
#' [see here for more details](https://pbreheny.github.io/ncvreg/articles/models.html).
#'
#' For `family="binomial"` models, the cross-validation fold assignments are balanced across the 0/1
#' outcomes, so that each fold has the same proportion of 0/1 outcomes (or as close to the same
#' proportion as it is possible to achieve if cases do not divide evenly).
#'
#' For Cox models, `cv.ncvsurv()` uses the approach of calculating the full Cox partial likelihood
#' using the cross-validated set of linear predictors. Other approaches to cross-validation for the
#' Cox regression model have been proposed in the literature; the strengths and weaknesses of the
#' various methods for penalized regression in the Cox model are the subject of current research. A
#' simple approximation to the standard error is provided, although an option to bootstrap the
...
#' @param cluster `cv.ncvreg()` and `cv.ncvsurv()` can be run in parallel across a cluster using the
#'   **parallel** package. The cluster must be set up in advance using the [parallel::makeCluster()]
#'   function from that package. The cluster must then be passed to `cv.ncvreg()` or `cv.ncvsurv()`
#'   (see example).
#' @param nfolds The number of cross-validation folds. Default is 10.
#' @param fold Which fold each observation belongs to. By default the observations are randomly
#'   assigned.
#' @param returnY Should `cv.ncvreg()` /`cv.ncvsurv()` return the linear predictors from the
#'   cross-validation folds? Default is `FALSE`; if `TRUE`, this will return a matrix in which the
#'   element for row i, column j is the fitted value for observation i from the fold in which
#'   observation i was excluded from the fit, at the jth value of lambda. NOTE: For `cv.ncvsurv()`,
#'   the rows of `Y` are ordered by time on study, and therefore will not correspond to the original
#'   order of observations pased to `cv.ncvsurv()`.
#' @param trace If set to `TRUE`, inform the user of progress by announcing the beginning of each CV
#'   fold. Default is `FALSE`.
#' @param se For `cv.ncvsurv()`, the method by which the cross-valiation standard error (CVSE) is
#'   calculated. The 'quick' approach is based on a rough approximation, but can be calculated more
#'   or less instantly. The 'bootstrap' approach is more accurate, but requires additional computing
#'   time.
#'
...
#'   \item{fold}{
#'     The fold assignments for cross-validation for each observation; note that for `cv.ncvsurv()`,
#'     these are in terms of the ordered observations, not the original observations.
#'   }
#'   \item{lambda}{
...
#'   \item{null.dev}{
#'     The deviance for the intercept-only model. If you have supplied your own `lambda` sequence,
#'     this quantity may not be meaningful.
#'   }
#'   \item{Bias}{
#'     The estimated bias of the minimum cross-validation error, as in Tibshirani and Tibshirani
#'     (2009) \doi{10.1214/08-AOAS224}
#'   }
#'   \item{pe}{
...
cv.ncvreg <- function(X, y, ..., cluster, nfolds = 10, fold, returnY = FALSE, trace = FALSE) {
  ...
  # Bias correction
  e <- sapply(1:nfolds, function(i) {
    apply(E[fold == i, , drop = FALSE], 2, mean)
  })
