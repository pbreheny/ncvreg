...
#'
#' Plots the cross-validation curve from a `cv.ncvreg` or `cv.ncvsurv` object, along with standard
#' error bars.
#'
#' Error bars representing approximate 68% confidence intervals are plotted along with the estimates
#' across values of `lambda`. For `rsq` and `snr` applied to models other than linear regression,
#' the Cox-Snell R-squared is used.
#'
...
#'   - `scale` plots, for `family="gaussian"`, an estimate of the scale parameter
#'     (standard deviation)
#'   - `pred` plots, for `family="binomial"`, the estimated prediction error
#'   - `all` produces all of the above
#' @param selected If `TRUE` (the default), places an axis on top of the plot denoting the number of
#'   variables in the model (i.e., that have a nonzero regression coefficient) at that value of
#'   `lambda`.
#' @param vertical.line If `TRUE` (the default), draws a vertical line at the value where
...
plot.cv.ncvreg <- function(
  ...
  if (type == "all") {
    ...
    if (length(x$fit$family)) {
      if (x$fit$family == "binomial") {
        plot(x, log.l = log.l, type = "pred", selected = selected, ...)
      }
      if (x$fit$family == "gaussian") {
  ...
  if (type == "cve") {
    ...
    if (x$fit$family == "binomial") {
      stop("Scale parameter for binomial family fixed at 1", call. = FALSE)
    }
    y <- sqrt(x$cve)
    L <- sqrt(L.cve)
    U <- sqrt(U.cve)
    ...
    y <- x$pe
    n <- x$fit$n
    CI <- sapply(y, function(x) {
      binom.test(x * n, n, conf.level = 0.68)$conf.int
    })
    L <- CI[1, ]
    U <- CI[2, ]
  ...
  ind <- if (type == "pred") {
    which(is.finite(l[1:length(x$pe)]))
  } else {
    which(is.finite(l[1:length(x$cve)]))
  }
