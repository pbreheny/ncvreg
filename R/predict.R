...
#'
#' Similar to other predict methods, this function returns predictions from a fitted `ncvreg`
#' object.
#'
...
#'   - `response` gives the fitted values (i.e., with the inverse link transformation applied)
...
#'   \item{`type = "response"`}{A numeric matrix of fitted values.}
#'   \item{`type = "class"`}{
#'     A numeric matrix of 0/1 values indicating the class with the highest probability.
#'   }
...
predict.ncvreg <- function(
  ...
  resp <- switch(object$family, binomial = exp(eta) / (1 + exp(eta)), poisson = exp(eta))
  if (type == "response") {
    return(drop(resp))
  }
  if (type == "class") {
