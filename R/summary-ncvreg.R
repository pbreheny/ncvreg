...
#'
#' Inferential summaries for `ncvreg` and `ncvsurv` objects based on local marginal false discovery
#' rates.
#'
...
#' @param which Alternatively, `lambda` may be specified by index; `which=10` means: report
#'   inference for the 10th value of `lambda` along the regularization path. If both `lambda` and
#'   `which` are specified, `lambda` takes precedence.
#' @param number By default, `summary` will provide an inferential summary for each variable that
#'   has been selected (i.e., each variable with a nonzero coefficient). Specifying `number=5`, for
#'   example, means that the summary table will include the 5 features with the lowest mfdr values,
#'   regardless of whether they were selected. To see all features, `number=Inf`.
#' @param cutoff Alternatively, specifying for example `cutoff=0.3` will report inference for all
#'   features with mfdr under 30%. If both `number` and `cutoff` are specified, the intersection
...
#' @param ... Further arguments; in particular, if you have set `returnX=FALSE`, you will need to
#'   supply `X` and `y` in order to calculate local mFDRs.
#'
#' @returns
#' An object with S3 class `summary.ncvreg`. The class has its own print method and contains the
...
#'   }
#'   \item{table}{
#'     A table containing estimates, normalized test statistics (z), and an estimate of the local
#'     mfdr for each coefficient. The mfdr may be loosely interpreted, in an empirical Bayes sense,
#'     as the probability that the given feature is null.
#'   }
#'   \item{unpen.table}{
#'     If there are any unpenalized coefficients, a separate inferential summary is given for them.
#'     Currently, this is based on `lm` /`glm`/`coxph` using the penalized coefficients to provide
#'     an offset. This is useful and more or less accurate, but not ideal; we hope to improve the
#'     inferential methods for unpenalized variables in the future.
#'   }
#' }
#'
...
summary.ncvreg <- function(object, lambda, which, number, cutoff, sort = TRUE, sigma, ...) {
  ...
  # Sort/subset table
  if (!missing(number) && number > nrow(Tab)) {
    number <- nrow(Tab)
  }
...
print.summary.ncvreg <- function(x, digits, ...) {
  ...
  if (!skip_intro) {
    ...
    if (x$custom) {
      ...
      cat(
        "  Average mfdr among chosen features : ",
        formatC(mean(x$table$mfdr), digits = 3),
        "\n",
        sep = ""
      )
    } else {
  ...
  if (!x$custom) {
    if (x$nvars > 0) {
      cat(
        ...
        formatC(sum(x$table$mfdr), digits = 2, format = "f", width = 6),
        "\n",
        sep = ""
      )
      ...
      cat(
        ...
        space,
        ": ",
        formatC(mean(x$table$mfdr), digits = 3, format = "f", width = 7),
        "\n",
        sep = ""
      )
    }
