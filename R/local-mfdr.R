...
local_mfdr <- function(fit, lambda, X = NULL, y = NULL, method = c("ashr", "kernel"), sigma, ...) {
  ...
  if (is.null(X) & is.null(fit$X)) {
    stop(
      "This procedure requires X and y.  Either supply X and y, or fit the model using the option 'returnX = TRUE'",
      call. = FALSE
    )
  }
  ...
  if (!inherits(fit, "ncvsurv")) {
    if (fit$family == "gaussian") {
      ...
      # Setup the score vector and weights
      pi <- 1 / (1 + exp(-a - (XX %*% bb)))
      U <- yy - pi
      w <- as.vector(pi * (1 - pi))
  ...
  if (inherits(fit, "ncvsurv")) {
    ...
    pi <- outer(exp(eta), rsk, "/")
    pi[upper.tri(pi)] <- 0
    U <- d - pi %*% d
    W <- -pi %*% diag(d) %*% t(pi)
    diag(W) <- diag(pi %*% diag(d) %*% t(1 - pi))
  ...
  if (!inherits(fit, "ncvsurv")) {
    ...
    unpen.res <- data.frame(
      Estimate = beta[-1][ns][!pen.idx],
      std.error = unpen.res[-1, 2] / sc[ns][!pen.idx],
      statistic = unpen.res[-1, 3],
      p.value = unpen.res[-1, 4]
    )
    rownames(unpen.res) <- names(bb)[!pen.idx]
  } else {
    off <- XX[, pen.idx] %*% bb[pen.idx]
    unpen.res <- summary(survival::coxph(
      survival::Surv(y, d) ~ XX[, !pen.idx] + offset(off)
    ))$coefficients
    unpen.res <- data.frame(
      Estimate = unpen.res[, 1] / sc[ns][!pen.idx],
      std.error = unpen.res[, 3] / sc[ns][!pen.idx],
      statistic = unpen.res[, 4],
      p.value = unpen.res[, 5]
    )
    rownames(unpen.res) <- names(bb)[!pen.idx]
  }
