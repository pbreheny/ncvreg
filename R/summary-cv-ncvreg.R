...
#' @param include_fit In addition to summarizing the cross-validation object, also summarize the
#'   model with the lowest CV error? (default: FALSE)
#' @param \dots Further arguments passed to or from other methods.
#'
#' @returns An object with S3 class `summary.cv.ncvreg`. The class has its own print method and
...
#'   \item{r.squared}{
#'     Proportion of variance explained by the model, as estimated by cross-validation. For models
#'     outside of linear regression, the Cox-Snell approach to defining R-squared is used.
#'   }
...
summary.cv.ncvreg <- function(object, include_fit = FALSE, ...) {
  ...
  if (!inherits(object, "cv.ncvsurv") && object$fit$family == "binomial") {
    val$pe <- object$pe
  }
...
print.summary.cv.ncvreg <- function(x, digits, include_fit = FALSE, ...) {
  ...
  if (missing(include_fit)) {
    include_fit <- x$include_fit
  }
  ...
  if (x$model == "logistic") {
    cat("  Prediction error: ", formatC(x$pe[x$min], digits[5], format = "f"), "\n", sep = "")
  }
  if (x$model == "linear") {
    cat(
