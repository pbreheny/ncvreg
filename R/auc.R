...
#'
#' The area under the curve (AUC), or equivalently, the concordance statistic (C), is calculated
#' according to the procedure described in van Houwelingen and Putter (2011). The function calls
#' [survival::concordancefit()], except cross-validated linear predictors are used to guard against
#' overfitting. Thus, the values returned by `AUC.cv.ncvsurv()` will be lower than those you would
#' obtain with `concordancefit()` if you fit the full (unpenalized) model.
#'
#' @param obj A `cv.ncvsurv` object. You must run [cv.ncvsurv()] with the option `returnY=TRUE` in
...
AUC.cv.ncvsurv <- function(obj, ...) {
  if (!("Y" %in% names(obj))) {
    stop("Must run cv.ncvsurv with 'returnY=TRUE' in order to calculate AUC", call. = FALSE)
  }
  if (!requireNamespace("survival", quietly = TRUE)) {
    stop("The 'survival' package is needed for AUC() to work. Please install it.", call. = FALSE)
  }
  if (utils::packageVersion("survival") < "3.2.10") {
    stop("AUC.cv.ncvsurv requires version 3.2.10 of 'survival' package or higher", call. = FALSE)
  }
