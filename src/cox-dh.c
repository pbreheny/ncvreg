...
SEXP cleanupCox(double *a, double *r, double *h, int *e, double *eta,
                double *haz, double *rsk, SEXP beta, SEXP Loss, SEXP iter,
                SEXP Eta) {
  free(a);
  free(r);
  free(h);
...
SEXP cdfit_cox_dh(SEXP X_, SEXP d_, SEXP penalty_, SEXP lambda, SEXP eps_,
                  SEXP max_iter_, SEXP gamma_, SEXP multiplier, SEXP alpha_,
                  SEXP dfmax_, SEXP user_, SEXP warn_) {
        ...
        for (int j = 0; j < p; j++) {
          if (e[j]) {
            ...
            if (shift != 0) {
              ...
              if (fabs(shift) * sqrt(v) > maxChange)
