...
SEXP cleanup_gaussian(double *a, double *r, int *e1, int *e2, double *z,
                      SEXP beta, SEXP loss, SEXP Eta, SEXP iter) {
  free(a);
  free(r);
  free(e1);
  free(e2);
...
SEXP cdfit_gaussian(SEXP X_, SEXP y_, SEXP penalty_, SEXP lambda, SEXP eps_,
                    SEXP max_iter_, SEXP gamma_, SEXP multiplier, SEXP alpha_,
                    SEXP dfmax_, SEXP user_) {
      ...
      while (tot_iter < max_iter) {
        while (tot_iter < max_iter) {
          ...
          for (int j = 0; j < p; j++) {
            if (e1[j]) {
              ...
              if (strcmp(penalty, "MCP") == 0)
                b[l * p + j] = MCP(z[j], l1, l2, gamma, 1);
              if (strcmp(penalty, "SCAD") == 0)
                b[l * p + j] = SCAD(z[j], l1, l2, gamma, 1);
              if (strcmp(penalty, "lasso") == 0)
        ...
        for (int j = 0; j < p; j++) {
          if (e1[j] == 0 && e2[j] == 1) {
            ...
            // If something enters the eligible set, update eligible set &
            // residuals
            if (b[l * p + j] != 0) {
              e1[j] = e2[j] = 1;
              for (int i = 0; i < n; i++)
