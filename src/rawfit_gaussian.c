...
SEXP cleanupRawG(double *a, double *v, double *z, int *active, SEXP beta,
                 SEXP loss, SEXP iter, SEXP resid) {
  free(a);
  free(v);
  free(z);
  free(active);
...
SEXP rawfit_gaussian(SEXP X_, SEXP y_, SEXP init_, SEXP r_, SEXP xtx_,
                     SEXP penalty_, SEXP lambda, SEXP eps_, SEXP max_iter_,
                     SEXP gamma_, SEXP multiplier, SEXP alpha_) {
      ...
      for (int j = 0; j < p; j++) {
        if (active[j]) {
          ...
          if (strcmp(penalty, "MCP") == 0)
            b[j] = MCP(z[j], l1, l2, gamma, v[j]);
          if (strcmp(penalty, "SCAD") == 0)
            b[j] = SCAD(z[j], l1, l2, gamma, v[j]);
          if (strcmp(penalty, "lasso") == 0)
            b[j] = lasso(z[j], l1, l2, v[j]);
          ...
          if (shift != 0) {
            for (int i = 0; i < n; i++)
              r[i] -= shift * X[j * n + i];
            if (fabs(shift) * sqrt(v[j]) > maxChange)
              maxChange = fabs(shift) * sqrt(v[j]);
          }
        }
      }
