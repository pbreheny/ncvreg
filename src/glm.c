...
SEXP cleanup_glm(double *s, double *w, double *a, double *r, int *e1, int *e2,
                 double *z, double *eta, SEXP beta0, SEXP beta, SEXP Dev,
                 SEXP Eta, SEXP iter) {
  free(s);
  free(w);
  free(a);
...
SEXP cdfit_glm(SEXP X_, SEXP y_, SEXP family_, SEXP penalty_, SEXP lambda,
               SEXP eps_, SEXP max_iter_, SEXP gamma_, SEXP multiplier,
               SEXP alpha_, SEXP dfmax_, SEXP user_, SEXP warn_) {
        ...
        while (tot_iter < max_iter) {
          ...
          if (strcmp(family, "binomial") == 0) {
            ...
            for (int i = 0; i < n; i++) {
              ...
              r[i] = s[i] / w[i];
              if (y[i] == 1)
                REAL(Dev)[l] = REAL(Dev)[l] - log(mu);
              if (y[i] == 0)
                REAL(Dev)[l] = REAL(Dev)[l] - log(1 - mu);
            }
          } else if (strcmp(family, "poisson") == 0) {
            for (int i = 0; i < n; i++) {
              mu = exp(eta[i]);
